...
export const SKILLS = [
  ...
  },
  {
    ...
    author: "Zinit",
    description: "Turns raw spend data into a prioritized savings pipeline with dollar-quantified levers, competitive sourcing recommendations, and executive-ready reporting — in minutes instead of weeks.",
    long_description: "Ingests procurement transaction data — purchase orders, invoices, or spend exports — and applies McKinsey/AT Kearney savings methodology across price harmonization, supplier consolidation, contract compliance, and category sourcing levers. Produces two coordinated deliverables: a 10-sheet Excel workbook with traceable analytics and a polished HTML report with interactive charts, an eRFx opportunity table, and a share-with-colleague button.",
    repo_url: null,
    installs: 0,
    stars: 0,
    badge: null,
    ...
    industries: ["cpg", "retail", "industrials", "logistics"],
    functions: ["procurement"],
    use_cases: [
      {
        ask: "Here's our spend data — find where we're overpaying",
        returns: "10-sheet Excel workbook with price harmonization analysis, savings pipeline ranked by impact, sole-source risk flags, and two should-cost models"
      },
      {
        ask: "Which categories should we take to competitive bid?",
        returns: "eRFx & eAuction opportunity table identifying categories ready for competitive sourcing events, with estimated savings and recommended actions"
      },
      {
        ask: "I need something I can share with leadership today",
        returns: "Interactive HTML report with KPI dashboard, animated charts, savings timeline, and a pre-filled Share button for one-click email distribution"
      }
    ],
    inputs: [
      {
        label: "Spend data file",
        detail: "Excel or CSV with transaction-level detail: line items, categories, suppliers, quantities, unit prices, and totals",
        required: true
      },
      {
        label: "Contract register",
        detail: "Supplier contracts with pricing, dates, volume commitments, and rebate terms. Upgrades contract leakage analysis from hypothesis to quantified mode",
        required: false
      },
      {
        label: "Client name",
        detail: "Used for file naming and report branding",
        required: false
      }
    ],
    outputs: [
      {
        ...
        detail: "Summary pipeline, spend overview, price harmonization detail, supplier analysis, contract leakage, sole-source risk register, quick wins, full savings pipeline, design-to-value hypotheses with should-cost models, and raw data"
      },
      {
        label: "Interactive HTML report",
        detail: "Executive dashboard with KPI cards, savings timeline, pipeline table, eRFx opportunity table, animated bar charts, and floating share button. Self-contained single file, opens in any browser"
      }
    ],
    how_it_works: [
      {
        title: "Row-level price benchmarking",
        detail: "Compares each transaction's unit price against the best price paid for the same item across all sites, flagging only lines where the overcharge exceeds 10% to avoid false positives."
      },
      {
        title: "Category-calibrated savings rates",
        detail: "Applies industry-standard savings percentages that vary by category (8–12%), reflecting that MRO parts, logistics, and office supplies each respond differently to competitive bidding."
      },
      {
        title: "Non-additive sub-lever architecture",
        detail: "Tracks which savings actions are subsets of larger initiatives and prevents double-counting in the grand total."
      },
      {
        title: "Dual-mode contract analysis",
        detail: "When no contract register is provided, frames all compliance findings as hypotheses. When contracts are uploaded, automatically quantifies four leakage types: price overcharges, post-expiry spend, off-contract purchases, and missed rebate tiers."
      },
      {
        title: "Should-cost decomposition",
        detail: "Selects high-volume, high-value items and builds component cost models (material, labor, process, overhead, margin, freight) to identify negotiation anchors for supplier discussions."
      },
      {
        title: "Cross-deliverable consistency",
        detail: "Both the Excel workbook and the HTML report draw from the same analytical engine, ensuring every dollar figure, lever ranking, and recommendation matches across formats."
      }
    ],
    tools: "Works standalone — no external APIs, databases, or procurement systems required. Reads uploaded Excel or CSV files directly. Output files are self-contained and can be opened in Excel and any web browser.",
    skills_summary: "Single-skill plugin. The Procurement Spend Analyzer skill is the sole invocable skill — it orchestrates the full analytical workflow from data ingestion through dual-deliverable output.",
    install_steps: [
      "Download from https://zinit.com/en/spend-analysis"
    ]
  }
];
